...
from impls import softmax

train_x = np.loadtxt('.data/iris-train-x.txt').astype(np.float32)
train_y = np.loadtxt('.data/iris-train-y.txt').astype(np.int32)
train_y = np.array(np.argmax(train_y, axis=1)).astype(np.int32)  # 元のデータフォーマットに戻す
...
# 評価
pred_y = model.classify(Variable(test_x, volatile=True)).data
